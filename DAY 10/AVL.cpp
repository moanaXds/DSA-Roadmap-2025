...
public:
    ...
    bool VERIFY_AVL_HELPER(Node* node)
    {
        if (!node) return true;
        ...
        return VERIFY_AVL_HELPER(node->left) && VERIFY_AVL_HELPER(node->right);
    }

    bool verifyAVL()
    {
        return VERIFY_AVL_HELPER(root);
    }
...
{
    GamerAVL G1;
    G1.insert(Gamer(80, 85.5));
    G1.insert(Gamer(30, 70.2));
    G1.insert(Gamer(70, 92.1));

    cout << "\n-----------------\n";
    G1.displayLeaderboard();

    G1.deleteMinAccuracy();
    G1.displayLeaderboard();
